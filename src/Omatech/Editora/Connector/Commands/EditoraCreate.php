...
class EditoraCreate extends Command
{
    ...
    public function handle()
    {
        ...
        $this->line(shell_exec('php '.$command.' --from=file --inputformat=array --inputfile='.$arraydatabase.' --to=db4 --dbtohost='.env('DB_CONNECTION').' --dbtouser='.env('DB_USERNAME').' --dbtopass='.env('DB_PASSWORD').' --dbtoname='.env('DB_DATABASE').''));
    }
